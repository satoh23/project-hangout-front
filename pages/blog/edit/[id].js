...
export default function BlogEdit() {
    ...
    const [blog, setBlog] = useState({thumbnail: "", encoded_thumbnail: "",
                                      title: "", summary: "", body: "", price: 0, genre: "", author: ""})
    ...
    useEffect(() => {
        const getBlogDetail = async() => {
            ...
            await fetch(`${process.env.NEXT_PUBLIC_RESTAPI_URL}api/v1/edit-blog/${id}/`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            .then((res) => {
                setBlog({thumbnail: res.thumbnail, encoded_thumbnail: res.encoded_thumbnail,
                         title: res.title, summary: res.summary, body: res.body, price: Math.ceil(res.price/11*10), genre: res.genre_id, author: res.author_id})
            })
        };
        getBlogDetail();
        mutate();
    ...
    const editBlog = async (e) => {
        ...
        if (isChangeFile) {
            await fetch(
                `${process.env.NEXT_PUBLIC_RESTAPI_URL}api/v1/edit-blog/${id}/`,
                {
                method: "PATCH",
                body: JSON.stringify({thumbnail: blog.thumbnail, encoded_thumbnail: blog.thumbnail, title: blog.title,
                                      summary: blog.summary, body: blog.body, price: price, author_id: blog.author, genre_id: blog.genre}),
                headers: {
                    "Content-Type": "application/json",
                },
                credentials: "include"
            ...
            await fetch(
                ...
                {
                  method: "PATCH",
                  body: JSON.stringify({thumbnail: blog.encoded_thumbnail, encoded_thumbnail: blog.encoded_thumbnail, title: blog.title, 
                                        summary: blog.summary, body: blog.body, price: price, author_id: blog.author, genre_id: blog.genre}),
                  headers: {
                    "Content-Type": "application/json",
                  },
              ...
              if (wasGetNewToken) {
                ...
                await fetch(
                    `${process.env.NEXT_PUBLIC_RESTAPI_URL}api/v1/edit-blog-not-thumbnail/${id}/`,
                    {
                      method: "PATCH",
                      body: JSON.stringify({title: blog.title, summary: blog.summary, body: blog.body, price: price, author_id: blog.author, genre_id: blog.genre}),
                      headers: {
                        "Content-Type": "application/json",
                      },
                      credentials: "include"
    ...
    return (
        ...
        <div className="w-full sm:w-3/4 lg:w-2/4 mb-10 md:mb-0 text-center">
                <div>
                    <img
                    src={previwThumbnail ? previwThumbnail : blog.thumbnail ? blog.thumbnail : "/NoImage.jpg"} 
                    alt="icon" 
                ...
                <div className="w-full flex">
                        ...
                        onChange={(e) => setBlog({...blog, genre: e.target.value})}>
                            ...
                            {data &&
                            data.map((genre) => genre.id===blog.genre ? <option key={genre.id} value={genre.id} selected>{genre.genre}</option> : <option key={genre.id} value={genre.id}>{genre.genre}</option> )}
                        </select>
                </div>
        ...
        <nav className='w-full flex bg-opacity-80 items-center flex-wrap bg-pink-200 font-nav z-10 fixed bottom-0 lg:hidden block border-t border-powder-pink'>
            <div className='flex w-full overflow-hidden flex-row h-auto content-center p-2'>
                    ...
                    <button
                        type="submit"
                        className="ml-5 mr-3 bg-transparent font-bold bg-pink-300 hover:bg-white text-white hover:text-pink-300 hover:text-white py-2 px-3 rounded">
                            更新する
                    </button> 
            </div>
        </nav>
